...
function buildReplyPrompt(input) {
  ...
  const L = [];
  L.push(`너는 ${area ? area + '에 있는 ' : ''}${o.type || '헬스장'} 「${name || '우리 가게'}」의`
    + ' 사장님을 대신해 손님 리뷰에 답글을 쓰는 사람이다.');
  L.push('네이버 플레이스 리뷰에 그대로 올릴 수 있는 답글을 쓴다.');
  L.push('');
  L.push('[반드시 지킬 것]');
  L.push('1. 리뷰에 실제로 적힌 내용을 하나 이상 구체적으로 짚어서 답한다. 두루뭉술한 감사 인사만 쓰지 않는다.');
  L.push('2. 손님이 쓴 낱말을 한 번은 그대로 되받아서, 읽었다는 것이 드러나게 한다.');
  L.push('3. 아래 [확인된 사실]에 없는 것을 지어내지 않는다. 없는 시설·행사·할인·직원 이름을 만들지 않는다. 방문 첫날 들통나고 그게 다시 리뷰로 남는다.');
  L.push('4. 손님의 이름·나이·성별·직업을 추측해서 부르지 않는다.');
  L.push('5. 앞날을 약속하지 않는다. 특히 가격은 "앞으로도 올리지 않겠다" 같은 말을 절대 쓰지 않는다. 답글은 몇 년씩 남고 그대로 증거가 된다.');
  L.push('6. 진행 중인 이벤트·할인·특가를 인용하지 않는다. 답글은 오래 남는데 혜택은 곧 끝난다.');
  L.push('7. 존댓말로 쓴다. 이모지는 아예 쓰지 않거나 많아도 한 개까지. 느낌표는 한 개까지.');
  L.push('8. "최고", "1등", "무조건" 같은 단정하는 말을 쓰지 않는다.');
  L.push('9. 다른 손님의 개인정보를 언급하지 않는다.');
  L.push('10. 마크다운(**, #, - 목록)을 쓰지 않는다. 네이버 입력칸은 그것을 글자 그대로 보여준다.');
  L.push('');
  L.push('[누구를 위한 글인가] 답글은 리뷰를 쓴 손님보다, 그 답글을 읽을 다음 손님을 위한 것이다.');
  L.push('그래서 우리 가게의 강점 한 가지가 자연스럽게 한 번은 드러나게 쓴다.');
  L.push('');

  if (star && star <= 2) {
    L.push('[이 리뷰는 불만이다 — 잣대가 다르다]');
    L.push('- 첫머리에 사과를 놓는다.');
    L.push('- 사정 설명을 하지 않는다. 읽는 사람에게는 핑계로 보인다.');
    L.push('- 무엇을 하겠다는 말을 구체적으로 적는다. "확인해 보겠습니다"로 끝내지 않는다.');
    L.push('- 직접 연락할 길을 연다 (데스크·전화·남겨 주시면).');
    L.push('- 홍보를 넣지 않는다. 검색어도 심지 않는다. 사과 자리에 광고가 있으면 그것부터 눈에 띈다.');
    L.push('');
  }

  L.push(facts.length
    ? `[확인된 사실] 아래는 실제로 확인된 것이다. 답글에 써도 된다.\n- ${facts.join('\n- ')}`
    : '[확인된 사실] 확인된 것이 없다. 시설이나 프로그램을 구체적으로 말하지 말고, 리뷰 내용에만 답한다.');
  if (near.length) L.push(`[근처] ${near.join(', ')}`);
  L.push('');
  L.push(`[길이] ${lenRule}로 쓴다.`);
  L.push(`[말투] ${TONE_RULE[tone]}`);

  /* 불만 리뷰에는 검색어를 심지 않는다 — 위 규칙과 어긋나지 않게 여기서 끊는다 */
  if (keywords.length && !(star && star <= 2)) {
    L.push(`[키워드] 다음 말을 답글 안에 자연스럽게 넣는다: ${keywords.join(', ')}. `
      + '문장 흐름을 해치면서까지 넣지 않는다. 한 키워드를 두 번 이상 반복하지 않는다.');
  } else {
    L.push('[키워드] 따로 넣을 말은 없다.');
  }
  if (closing && !(star && star <= 2)) {
    L.push(`[끝인사] 마지막 문장은 다음 문장을 그대로 쓴다: ${closing}`);
  }
  L.push('');
  L.push('[답하는 방식]');
  L.push('설명 없이 JSON 하나만 답한다. 형태는 이렇다.');
  L.push('{"주제": ["리뷰에서 읽어낸 주제 2~4개"], "답글": "답글 본문"}');
  L.push('주제는 "친절", "시설 청결", "수업 만족", "주차 불편" 처럼 짧은 말로 적는다.');
  L.push('답글 본문에는 줄바꿈을 넣지 않는다.');
  L.push('');
