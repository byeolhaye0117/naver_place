...
function buildReplyPrompt(input) {
  ...
  if (bad) {
    ...
    L.push('3. 대응 — 손님이 말한 그 대목에 딱 맞는 [확인된 사실] 하나를 구체적으로 든다.');
    L.push('   예) "조용히 혼자" 라고 하셨으면 → 그 손님이 조용히 운동할 수 있는 이유를 사실로 댄다.');
    L.push('   두루뭉술한 자랑이 아니라, 그 손님의 말에 대한 답이어야 한다.');
    L.push('4. 다음 — 다음에 오시면 해보시라고 권할 것을 한 가지. [확인된 사실]에 있는 것만.');
    L.push('5. 마무리 — 짧은 응원이나 인사.');
  }
  L.push('');
  ...
  if (closing && !bad) L.push(`[끝인사] 마지막 문장은 다음 문장을 그대로 쓴다: ${closing}`);
  L.push('');
  L.push('[답하는 방식]');
